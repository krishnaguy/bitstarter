...
var checkHtmlUrl = function(urlres,checksfile) {
    $ = cheerio.load(urlres);
    var checks = loadChecks(checksfile).sort();
    var out = {};
    for (var ii in checks) {
...
//var x = cheerio.load(tmp['request']['res']['rawEncoded'])
//var checksFile = "checks.json";

//var checks = loadChecks(checksFile).sort();
//var checkJson = checkHtmlUrl(tmp, checksFile);
...
//	console.log("Calling URL");
	...
	var tmp = rs.get(program.url).on('complete',function(result){
		...
		$ = cheerio.load(result);
		var checks = loadChecks(checksFile).sort();
		//	console.log("Checks File: "+checksFile);
		//	console.log("result");
		//	console.log(result);
